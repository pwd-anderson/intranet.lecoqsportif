...
class SalesByBoutiquesX3Kpi
{
    ...
    public function __construct(
        ...
        private RfcByBoutiqueX3 $rfcByBoutique,
    ...
    public function getSalesByBoutique(int $year, int $week, string $businessType = 'CONCEPT STORE'): array
    {
        return [
    ...
    private function buildPeriod(int $year, int $week, bool $cumul, string $businessType = 'CONCEPT STORE'): array
    {
        /**
        ...
        $label = $businessType === 'CONCEPT STORE'
            ? 'CONCEPT STORE'
            : 'FACTORY OUTLET';
...
// MARGE
    ...
    private function fetchBoutiquesData(
        ...
        SELECT
            ...
            SUM(CASE WHEN YEAR(I.DOCUMENTPOSTINGDATE) = {$yearMinus1}
                THEN I.AMOUNTEURTM ELSE 0 END) AS Amount_N1,

            SUM(CASE WHEN YEAR(I.DOCUMENTPOSTINGDATE) = {$year}
                THEN I.MARGINAMOUNTEURTM ELSE 0 END) AS Margin_N,

            SUM(CASE WHEN YEAR(I.DOCUMENTPOSTINGDATE) = {$yearMinus1}
                THEN I.MARGINAMOUNTEURTM ELSE 0 END) AS Margin_N1

        FROM SEI_X3_LCS.CONSO_INVOICES I
        LEFT JOIN X3_LCS.FACILITY FCY ON I.LOCATIONCODE = FCY.FCY_0
            ...
            AND (
                CASE
                    WHEN DATEPART(ISO_WEEK, I.DOCUMENTPOSTINGDATE) = 1
                         AND MONTH(I.DOCUMENTPOSTINGDATE) = 12
                        THEN YEAR(I.DOCUMENTPOSTINGDATE) + 1

                    WHEN DATEPART(ISO_WEEK, I.DOCUMENTPOSTINGDATE) >= 52
                         AND MONTH(I.DOCUMENTPOSTINGDATE) = 1
                        THEN YEAR(I.DOCUMENTPOSTINGDATE) - 1
    ...
    ";
        ...
        if ($cumul) {
            $sql .= " AND DATEPART(ISO_WEEK, I.DOCUMENTPOSTINGDATE) <= {$week} ";
        } else {
            $sql .= " AND DATEPART(ISO_WEEK, I.DOCUMENTPOSTINGDATE) = {$week} ";
        }
