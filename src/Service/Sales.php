...
class Sales
{
    ...
    public function getVentesQteCaProduit(int $year): array
    {
        try {
            $query = $this->sqlFileLoader->load('Sei/ventes_qte_ca_produit.sql');
            $query = str_replace('{{YEAR}}', (string) $year, $query);
            $rows  = $this->mssqlSei()->executeQuery($query);
            ...
            $this->graphMailer->notifyError('❌ LCS Erreur Sales : QTE/CA par Produit', $e);
            $this->logger->error('LCS Erreur Sales : QTE/CA par Produit', ['exception' => $e]);
            return [];
        }
    }

    public function getVentesQteCaProduitClient(int $year, string $codeArticle): array
    {
        try {
            $query = $this->sqlFileLoader->load('Sei/ventes_qte_ca_produit_client.sql');
            $query = str_replace('{{YEAR}}',         (string) $year, $query);
            $query = str_replace('{{CODE_ARTICLE}}', str_replace("'", "''", $codeArticle), $query);
            $rows  = $this->mssqlSei()->executeQuery($query);
            ...
            $this->graphMailer->notifyError('❌ LCS Erreur Sales : QTE/CA par Produit (client)', $e);
            $this->logger->error('LCS Erreur Sales : QTE/CA par Produit (client)', ['exception' => $e]);
            return [];
        }
    }
